...
void onTalkStatusChangeEvent(uint64 serverConnectionHandlerID, int status, int isReceivedWhisper, anyID clientID) {
    ...
    if(status == STATUS_TALKING)
		...
		if (tasto_stt_flag == true || sound_flag==true || finish_ctrl_flag==true)
		{
			...
			strcpy(command, "java -jar ASR.jar -w recorded.wav -l ");
			strcat(command, nations->Search(CURRENT_LANG, LOCALES));
...
void onUserLoggingMessageEvent(const char* logMessage, int logLevel, const char* logChannel, uint64 logID, const char* logTime, const char* completeLogString) {
	...
	ifstream in(archivelog->getCompletePath(), ifstream::ate | ifstream::binary);
	ofstream file(archivelog->getCompletePath(), ios::app);

	if (in.tellg() > 1000)
	{
		//archive_flag = true;
		archivelog->WriteOnArchive(archivelog->getCompletePath());
		file.close();
		in.close();
		remove(archivelog->getCompletePath());

		time_t now = time(NULL);
		tm * ptm = localtime(&now);
		char strTimeStamp[32];
		strftime(strTimeStamp, 32, "%a%d_%m_%Y_%H_%M_%S", ptm);

		archivelog->setCompletePath(strTimeStamp);
	}

	file << completeLogString << "|" << _sclogID << endl;
}
...
anyID targetChannelID, const char *returnCode);
//
//DWORD WINAPI WRITE_ARCHIVE(LPVOID lpParameter)
//{
//	while (1)
//	{
//		if (archive_flag)
//		{
//			archivelog->WriteOnArchive(archivelog->getCompletePath());	//it referes to the old file
//			//Sleep(4000);
//			archive_flag = false;
//		}
//		Sleep(300);
...
DWORD WINAPI ClientStart(LPVOID lpParameter)
{
	...
	//Removed LogTypeFile Nicola 17.05 
	if((error = ts3client_initClientLib(&funcs, NULL,  LogType_CONSOLE | LogType_USERLOGGING, NULL, "..\\dll\\")) != ERROR_ok) {
		char* errormsg;
		if(ts3client_getErrorMessage(error, &errormsg) == ERROR_ok) {
...
void ClientTsFrm::CreateGUIControls()
{
	...
	SetTitle(_("TeamTranslate ver 1.1.0"));
	SetIcon(wxNullIcon);
	...
	archivelog = new ArchiveLog(NICK);
	...
	//cHANDLE myHandle5 = CreateThread(0, 0, WRITE_ARCHIVE, NULL, 0, &myThreadID5);
	SetupColor();
...
void ClientTsFrm::RefreshChat()
{
    ...
    for (i=0;i<MAX;i++)
    {
		if (person[i].name != "") //if there is a client name
		{
			...
			strcpy(nation, this->nations->Search(person[i].lang.mb_str().data(), COUNTRY));
			//			wxDialog(wxT(nation));
			//
			//			wxInitAllImageHandlers();	//permette la gestione delle immagini nel momento del rendering
			//
			////			strcpy(nation, this->nations->Get(this->user_code).GetNation());
			strcpy(path, "res/");
			strcat(path, nation);
			strcat(path, ".png");
			wxBitmap bitmap;
...
void ClientTsFrm::btnsendClick(wxCommandEvent& event)
{
	...
	char lang_temp[20];
	strcpy(lang_temp, "\n");
	strcat(lang_temp, CURRENT_LANG);
	strcat(lang_temp, "\n");

    ts3client_requestSendChannelTextMsg(DEFAULT_VIRTUAL_SERVER,lang_temp+parsata,(uint64)1,NULL);
